...
const AssignStaffModal = ({
  ...
  useEffect(() => {
    ...
    getUsers({
      ...
      .then((res) => {
        if (!cancelled) {
          const list = ((res.data ?? []) as StaffUser[]).filter(
            (u) => u.role === "STAFF",
          );
          setStaff(list);
        }
            ...
            <ul className="space-y-2">
              {staff.map((user) => {
                ...
                return (
                  <li
                    key={user.id}
                    className="flex items-center justify-between gap-4 p-4 rounded-xl border border-gray-100 hover:border-primary/25 hover:bg-primary/5 transition-colors"
                  >
                    ...
                    <button
                      ...
                      className="shrink-0 px-4 py-2 rounded-xl bg-primary text-white text-sm font-bold hover:bg-primary/90 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
                    >
                      {isAssigning ? (
